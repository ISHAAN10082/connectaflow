...
import {
    Sparkles, BarChart3, Radio, Target, Upload, Download,
    Zap, ChevronRight, Settings, BookOpen
} from 'lucide-react';
...
export function ControlPanel() {
    ...
    return (
        <div className="flex h-screen bg-[#0A0F1E]" id="control-panel">
            {/* ── Sidebar ─────────────────────────────────────── */}
                    ...
                    {NAV_ITEMS.map(item => (
                        <button
                            ...
                            className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-left transition-all duration-200 group ${activeScreen === item.key
                                ? 'bg-gradient-to-r from-violet-600/15 to-indigo-600/10 text-white border border-violet-500/20'
                                : 'text-slate-400 hover:text-white hover:bg-white/[0.03]'
                                }`}
                        >
                            <item.icon className={`w-[18px] h-[18px] transition-colors ${activeScreen === item.key ? 'text-violet-400' : 'text-slate-500 group-hover:text-slate-300'
                                }`} />
